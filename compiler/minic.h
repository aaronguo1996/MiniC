...
#define minic_is_integer(type) ((type)->basic_type == MVM_INTEGER_TYPE && (type)->derive == NULL)

#define minic_is_decimal(type) ((type)->basic_type == MVM_DECIMAL_TYPE && (type)->derive == NULL)

#define minic_is_boolean(type) ((type)->basic_type == MVM_BOOLEAN_TYPE && (type)->derive == NULL)

#define minic_is_string(type) ((type)->basic_type == MVM_STRING_TYPE && (type)->derive == NULL)
...
typedef enum {
    ...
    INTEGER_TO_STRING_CAST,
    DECIMAL_TO_STRING_CAST
} CastType;
...
typedef struct MINIC_Compiler_tag {
    ...
    int			          mvm_function_count;
	MVM_Function          *mvm_function;
    int                   mvm_class_count;
	MVM_Class             *mvm_class;
    DeclarationList	      *declaration_list;
    StatementList	      *statement_list;
...
TypeSpecifier *minic_create_type_specifier(MVM_BasicType basic_type);
...
Expression *minic_create_boolean_expression(MVM_Boolean boolean_value);
...
Expression *minic_create_boolean_expression(MVM_Boolean value);
Expression *minic_create_null_expression(void);
Expression *minic_create_new_expression(char *class_name, char *name);
Expression *minic_create_basic_array_creation(MVM_BasicType basic_type, ArrayDimension *dim_expr, ArrayDimension *dim_ilst);
...
Expression *minic_create_array_creation(MVM_BasicType basic_type, ArrayDimension *dim_expr_list, ArrayDimension *dim_ilst);
...
/* generate.c */
MVM_TypeSpecifier *minic_copy_type_specifier(TypeSpecifier *src);
MVM_Executable *minic_generate(MINIC_Compiler *compiler);
...
TypeSpecifier *minic_alloc_type_specifier(MVM_BasicType type);
...
char *minic_get_basic_type_name(MVM_BasicType type);
char *minic_expression_to_string(Expression *expr);
char *minic_package_name_to_string(PackageName *src);
